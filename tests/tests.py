...
class DjangoConditionsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.mrx = User.objects.create_user(
            username="mr.x", email="x@gmail.com", password="top_secret"
        )
        cls.mrsy = User.objects.create_user(
            username="mrs.y", email="y@yahoo.com", password="also_top_secret"
        )
        ...
        cls.non_gmail_users_campaign = Campaign.objects.create(
            ...
            conditions={"all": ["NOT EMAIL_DOMAIN gmail.com"]},
        )
...
class TestEvalConditions(DjangoConditionsTestCase):
    ...
    def test_comparecondition_campaign(self):
        ...
        self.assertFalse(
            eval_conditions(self.comparecondition_campaign, "conditions", self.mrx)
        )
        self.assertFalse(
            eval_conditions(self.comparecondition_campaign, "conditions", self.mrsy)
        )
    ...
    def test_custom_operators(self):
        ...
        self.assertTrue(
            eval_conditions(self.long_term_user_campaign, "conditions", self.mrx)
        )
        self.assertFalse(
            eval_conditions(self.long_term_user_campaign, "conditions", self.mrsy)
        )

    def test_key(self):
        # Mr X. uses Gmail, but Mrs. Y doesn't
        self.assertFalse(
            eval_conditions(self.non_gmail_users_campaign, "conditions", self.mrx)
        )
        self.assertTrue(
            eval_conditions(self.non_gmail_users_campaign, "conditions", self.mrsy)
        )
    ...
    def test_nested_condlists(self):
        ...
        self.assertTrue(
            eval_conditions(
                self.long_term_gmail_yahoo_user_campaign, "conditions", self.mrx
            )
        )
        self.assertFalse(
            eval_conditions(
                self.long_term_gmail_yahoo_user_campaign, "conditions", self.mrsy
            )
        )
...
class TestAdmin(DjangoConditionsTestCase):
    ...
    def setUpTestData(cls):
        cls.admin_user = User.objects.create_superuser(
            username="admin", email="admin@example.com", password="admin_secret"
        )
